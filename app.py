...
from datetime import datetime, timedelta  # <--- Time fix ke liye timedelta add kia
...
# --- LOGIC HELPER FUNCTIONS (UPDATED FOR NEW COLUMNS) ---
def get_pending_status(m_data, driver_ids):
    pending_list = {}
    for idx, row in enumerate(m_data[5:], start=6): 
        # Ab Status Column P (Index 15) par hai
        if len(row) > 15:
...
def get_totals(m_data, driver_ids):
    ...
    for row in m_data[5:]: 
        # Ab Total Column O (Index 14) par hai
        if len(row) > 15:
            d_id = str(row[1]).strip()
            status = str(row[15])
    ...
    if st.button("START SESSION 🚀"):
        if s_amt is not None:
            ...
            row_data = [
                ...
                ksa_time.strftime("%m/%d/%Y"),  # Corrected Date
                s_amt, "", oil_val,
                ksa_time.strftime("%I:%M %p"),  # Corrected Time (+3 Hours)
                "", "", "", "", "", "", "Pending ⏳" # Empty columns for L, M, N, O
            ]
            
            # Start mein hum Column O (Total) aur P (Status) tak cells reserve kar rahe hain
            sheet.update(range_name=f"A{r_idx}:P{r_idx}", values=[row_data])
            st.cache_data.clear()
...
# --- PAGE: END FORM ---
    ...
    # --- NEW INPUT FOR PETROL ---
    petrol = st.number_input("PETROL / FOOD (EXPENSE)", value=0, placeholder="Enter Expense (if any)")
    ...
    if st.button("SAVE & FINISH ✅"):
        if e_amt is not None and cash is not None and bank is not None:
            ...
            # UPDATED FORMULA (Subtracting Petrol)
            total = cash + bank - id_cost - petrol
            
            # --- TIME FIX (+3 HOURS) ---
            end_time_str = (datetime.now() + timedelta(hours=3)).strftime("%I:%M %p")
            ...
            updates = [
                ...
                {'range': f'J{r}', 'values': [[end_time_str]]},
                {'range': f'K{r}', 'values': [[id_cost]]},
                {'range': f'L{r}', 'values': [[petrol]]},    # New Column for Petrol
                {'range': f'M{r}', 'values': [[bank]]},      # Bank shifted to M
                {'range': f'N{r}', 'values': [[cash]]},      # Cash shifted to N
                {'range': f'O{r}', 'values': [[total]]},     # Total shifted to O
                {'range': f'P{r}', 'values': [["Done ✔"]]}   # Status shifted to P
            ]
            sheet.batch_update(updates)
            
            st.session_state.res = {"h": cash, "b": bank, "id": id_cost, "p": petrol, "t": total}
            st.cache_data.clear()
...
# --- PAGE: RECEIPT ---
    ...
    st.markdown(f"""
        <div style='border:1px dashed #00ff41; padding:20px; font-family:monospace; background:#000; max-width:400px; margin:0 auto;'>
            ...
            <div style='display:flex; justify-content:space-between; color:#ff4444;'><span>Petrol/Food:</span><span>-{res['p']}</span></div>
